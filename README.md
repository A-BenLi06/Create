...
>    Do **not** keep both. 两个 jar **不能**同时存在。
> 4. Do the same on server and client — they must match.
>    服务端和客户端都要换，两边必须一致。
>
> Requires Minecraft 1.20.1 + Forge 47.1.33+. Same dependencies as stock Create 6.0.8.
> 需要 Minecraft 1.20.1 + Forge 47.1.33 及以上，依赖与原版 Create 6.0.8 相同。
>
> To go back, reinstall the official jar — no world data changes are involved.
> 想回退直接换回官方 jar 即可，本改动不涉及任何存档数据变更。
